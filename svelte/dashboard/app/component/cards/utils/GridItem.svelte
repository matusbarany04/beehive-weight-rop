<script>
    ...
    let pixelSize = spring({pixelHeight: item.pixelHeight, pixelWidth: item.pixelWidth}, {
        stiffness: 0.1,
        damping: 0.25
    })
    let itemCoords = spring({x: item.xCoordinate, y: item.yCoordinate}, {
        stiffness: 0.1,
        damping: 0.25
    })
    ...
    item.setValueChangedCallback(() => {
        ...
        pixelSize.set({pixelHeight: item.pixelHeight, pixelWidth: item.pixelWidth})
    });
