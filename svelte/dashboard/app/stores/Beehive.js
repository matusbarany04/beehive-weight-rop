...
export class BeehiveObj {
    ...
    constructor(beehive_id, name, location, model, connectionMode, interval, devices) {
        ...
        if (connectionMode === null || connectionMode === undefined) throw new Error("Connection mode is required.");
        if (interval === null || interval === undefined) throw new Error("Interval is required.");
    ...
    setData(type, data) {
        if (!Array.isArray(data)) {
            console.warn('Data provided is not an array. Setting it to an empty array.');
            this.data[type] = [];
        } else {
            this.data[type] = data;
        }
