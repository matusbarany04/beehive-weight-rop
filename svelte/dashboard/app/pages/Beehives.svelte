          ...
          {#each $rows as row, index}
            <tr class="h-20">
              ...
              <td>
                {#if row}
                  <div
                    class="box-content flex h-8 w-20 items-center justify-center rounded-full px-1 {
            
                        row.getCurrentStatus() === 'ok'
                        ? 'bg-confirm-200'
                        : 'bg-error-200'}"
                  >
                    <p
                      class="no_wrap text-ellipsis whitespace-nowrap font-semibold {
                         row.getCurrentStatus() === 'ok'
                          ? 'text-confirm-600'
                          : 'text-error-500'}"
                    >
