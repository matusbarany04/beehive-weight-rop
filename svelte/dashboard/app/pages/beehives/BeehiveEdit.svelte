...
let intervals = [[10, "10min"], [30, "30min"], [60, "1h"], [120, "2h"], [180, "3h"], [300, "5h"], [600, "10h"]];
...
<div class="w-full flex flex-row justify-center ">
  ...
  <form class="w-5/6" action="/dashboardApi/saveBeehive" method="post">
    <div class="bg-white rounded-lg p-4 m-4">
      <Input type="text" name="title" label="Názov váhy" value={beehive.name} inline required=""/>
      <Input type="text" name="location" label="Poloha" value={beehive.location} inline/>
      <DropdownInput name="interval" label="Interval merania" options={intervals} value={beehive.interval} inline />
    </div>
    ...
    <div class="bg-white rounded-lg m-4">
      ...
      <SelectableOption name="Mobilná sieť" bind:selection={connectionMode} value=0>
        <Input type="password" name="sim_password" label="Heslo na SIM kartu" value="0000" inline/>
      </SelectableOption>
      
      <SelectableOption name="WiFi" bind:selection={connectionMode} value=1>
        <Input type="text" name="wifi_ssid" label="Názov WiFi siete (SSID)" inline/>
        <Input type="password" name="wifi_password" label="Heslo" inline/>
      </SelectableOption>
      
      <SelectableOption name="Iná váha" bind:selection={connectionMode} value=2>
